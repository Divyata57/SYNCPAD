...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Call API directly to get the mock verification link in the response
      const apiRes = await api.post('/auth/register', { username, email, password });
      
      if (apiRes.data.success) {
        setSuccess(true);
        setMockLink(apiRes.data.mockVerificationLink);
      }
    } catch (err: any) {
        ...
        {success ? (
          <div className="space-y-4 text-center">
            ...
            <div className="space-y-2">
              <h3 className="font-bold text-sm">Verify Mock Account</h3>
              <p className="text-xs text-slate-400 leading-relaxed">
                We've simulated an email verification step. Click the button below to instantly verify and activate your profile.
              </p>
            </div>
            
            <a
              href={`http://localhost:5000${mockLink}`}
              target="_blank"
              rel="noopener noreferrer"
              className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs rounded-lg block shadow-lg shadow-emerald-500/25 transition-all cursor-pointer"
            >
              Verify Mock Email Account
            </a>

            <p className="text-[10px] text-slate-500">
              Note: Once clicked, return to log in with your credentials.
            </p>
            <div className="pt-2">
